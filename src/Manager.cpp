...
Manager::Manager(boost::asio::io_context& io_context, boost::shared_ptr<boost::asio::ssl::context> ssl_context): m_io_context(io_context) ,m_ssl_context(ssl_context),
...
void Manager::OnPageRequestEnd(boost::shared_ptr<WebRequest> wr, double time_spend)
{
...

	{
		...
		auto isLoaded = std::find(m_wr_loaded.begin(), m_wr_loaded.end(), wr);
		m_wr_loaded.erase(isLoaded);
	}
	...
	if (programShouldExit.load())
	{
		...
		if (m_pageQueue.size() > 0 && m_wr_loaded.size() < MAX_PAGES_IN_WORK)
		{
			while (m_pageQueue.size() > 0 && m_wr_loaded.size() < MAX_PAGES_IN_WORK)
			{
				...
				boost::asio::post(boost::asio::bind_executor(wr->GetStrand(), boost::bind(&WebRequest::LoadPage, wr->shared_from_this(), page, cb)));
			}
		}
